...
#
# Existing assets are kept: the script only adds (or overwrites) the archives for
# this version and regenerates checksums.txt for this version, so an older local
# release artifact is never deleted or included in the new release. Pass
# PRUNE_ASSETS=1 to remove archives that do not match VERSION first.
#
...
# skip platforms you cannot build. Linux targets must match the libc of the
# target container:
#   CC_linux_amd64="zig cc -target x86_64-linux-gnu.2.17"   # debian/ubuntu (official image)
#   CC_linux_amd64="zig cc -target x86_64-linux-musl"       # alpine
set -euo pipefail
...
if [ "${PRUNE_ASSETS:-0}" = "1" ]; then
  # Opt-in cleanup, restricted to this plugin's archives. checksums.txt is
  # regenerated below, so removing it is safe.
  find "$OUT_DIR" -maxdepth 1 -type f \( -name "${PLUGIN_ID}_*.zip" -o -name "checksums.txt" \) ! -name "${PLUGIN_ID}_${VERSION}_*" -delete
fi
...
for platform in $PLATFORMS; do
  ...
  stage="$(mktemp -d)"
  # Remove a previous archive for this exact platform so a failed build cannot
  # leave a stale zip next to a fresh checksum file.
  rm -f "$OUT_DIR/$ZIP_NAME"
  if ! CGO_ENABLED=1 GOOS="$GOOS_TARGET" GOARCH="$GOARCH_TARGET" \
       go build -trimpath -buildmode=c-shared -ldflags "-s -w -extldflags \"-Wl,-s\"" \
...
# Old archives may be present by design. Success requires at least one archive
# for the version requested by this run; an old zip must never hide a failed
# current build.
current_assets=("$OUT_DIR/${PLUGIN_ID}_${VERSION}_"*.zip)
if [ ! -e "${current_assets[0]}" ]; then
  echo "no platform was built for $VERSION; nothing to package" >&2
  exit 1
fi
...
if [ "$zip_tool" != "zip" ]; then
  OUT_DIR="$OUT_DIR" ASSET_PREFIX="${PLUGIN_ID}_${VERSION}_" "$zip_tool" -c "$(cat <<'PYSUM'
import hashlib, os
...
PYSUM
  ...
  ( cd "$OUT_DIR" && sha256sum ./${PLUGIN_ID}_${VERSION}_*.zip | sed 's#  \./#  #' > checksums.txt )
elif command -v shasum >/dev/null 2>&1; then
  ( cd "$OUT_DIR" && shasum -a 256 ./${PLUGIN_ID}_${VERSION}_*.zip | sed 's#  \./#  #' > checksums.txt )
else
